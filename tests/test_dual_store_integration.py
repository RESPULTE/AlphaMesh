...
async def test_dual_store_ingestion_writes_to_vector_and_graph_store() -> None:
    ...
    try:
        ...
        pytest.skip(
            f"Skipping integration test; required services not configured: {exc}"
        )
    ...
    try:
        ...
        exc_str = str(conn_exc).lower()
        exc_type = str(type(conn_exc).__name__).lower()
        if any(
            ...
            pytest.skip(f"Skipping; service connection failed: {conn_exc}")
        raise
    ...
    neo4j_data = await neo4j_adapter._execute_read(cypher_chunks, {"ids": chunk_ids})
    ...
    cypher_anchor = (
        "MATCH (d:Document)-[:ANCHORED_TO]->(g:GlobalAnchor {id: $anchor_id}) "
...
async def test_dual_store_ingestion_creates_multiple_chunks_when_overflowing() -> None:
    ...
    try:
        ...
        pytest.skip(
            f"Skipping overflow integration test; required services not configured: {exc}"
        )
    ...
    try:
        ...
        if any(
            ...
            pytest.skip(f"Skipping; service connection/async error: {conn_exc}")
        raise
    assert (
        len(chunk_ids) > 1
    ...
    neo4j_chunks = await neo4j_adapter._execute_read(cypher_chunks, {"ids": chunk_ids})
