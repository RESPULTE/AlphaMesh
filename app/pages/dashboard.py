...
from core.rag_handler import UserProfileRAG, generate_augmented_response
from core.services import get_llm, get_graph, get_vector_store

st.set_page_config(page_title="AlphaMesh Dashboard", layout="wide")

st.title("Welcome to your AlphaMesh Dashboard")
st.markdown(
    "This is your personalized investment intelligence hub. I can learn about your interests and goals over time."
)
...
if not all([llm, graph, vector_store]):
    st.error(
        "One or more backend services failed to initialize. The application cannot continue."
    )
...
def get_rag_handler(user_id, _llm, _graph, _vector_store):
    ...
    return UserProfileRAG(
        user_id=user_id, llm=_llm, graph=_graph, vector_store=_vector_store
    )
...
if prompt := st.chat_input(
    "Ask AlphaMesh anything... (e.g., 'My name is Jane and I'm interested in tech stocks')"
):
