...
class BaseAgentInput(BaseModel):
    ...
    query: str = Field(
        default="",
        description=(
            "Legacy query text. Reserved for orchestrator-level continuity and "
            "backward compatibility; sub-agents should rely on `goal`."
        )
    )
    goal: str = Field(
        default="",
        description=(
            "Plain-text execution goal for this agent generated by the orchestrator. "
            "Sub-agents should use this as the primary instruction for planning, "
            "checking, and final analysis."
        ),
    )
