...
def get_stock_news_urls(ticker: str, max_articles: int = 5) -> list[str]:
    """
    Fetches the latest news article URLs for a given stock ticker from Yahoo Finance.
    """
...
def create_retriever_for_stock(
    ...
    for new in news:
        try:
            text = article(new["content"]["clickThroughUrl"]["url"]).text
            ...
            summary = llm.invoke(
                prompt
            ).content  # adjust for your LLM’s interface (may be .predict or .invoke)

            docs.append(
                Document(
                    page_content=summary,
                    metadata={"title": new["content"]["title"]},
                )
            )
...
def run_analysis(ticker: str, question: str):
    ...
    retriever_tool = create_retriever_for_stock(
        ticker=ticker, llm=llm, embedding_function=embedding_func
    )
