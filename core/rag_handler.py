...
import core.rag.prompts as prompts
...
class VectorDoc(BaseModel):
    text: str = Field(
        description="The text content of the document to be added to the vector store."
    )
    entities: List[str] = Field(
        description="A list of key entities present in the text."
    )


class VectorUpdates(BaseModel):
    add_documents: List[VectorDoc] = Field(
        default_factory=list,
        description="List of new documents to add to the vector store.",
    )
    delete_by_entity: List[str] = Field(
        default_factory=list,
        description="List of entity names. All documents associated with these entities will be deleted.",
    )


class MemoryModification(BaseModel):
    graph_updates: List[str] = Field(
        default_factory=list, description="A list of Cypher statements to execute."
    )
    vector_updates: VectorUpdates = Field(
        default_factory=VectorUpdates,
        description="Instructions for updating the vector store.",
    )
...
class UserProfileRAG:
    ...
    def __init__(self, user_id: str, llm: BaseChatModel, graph, vector_store: Chroma):
        ...
        self.memory_modification_parser = JsonOutputParser(
            pydantic_object=MemoryModification
        )
    ...
    def _delete_vector_docs_by_entity(self, entity: str):
        ...
        try:
            ...
            docs_to_delete = self.vector_store.get(
                where={
                    "$and": [{"user_id": self.user_id}, {"entities": {"$in": entity}}]
                }
            )
            ...
            if ids_to_delete := docs_to_delete.get("ids"):
                ...
                logger.info(
                    f"Deleted {len(ids_to_delete)} vector documents for user {self.user_id} related to entity: '{entity}'"
                )
            else:
                logger.info(
                    f"No vector documents found to delete for entity: '{entity}'"
                )
        except Exception as e:
            logger.error(
                f"Error deleting vector docs for entity '{entity}' for user {self.user_id}: {e}"
            )
    ...
    def _consolidate_memories(self, conversation_turn: str):
        ...
        prompt = PromptTemplate(
            ...
            partial_variables={
                "format_instructions": self.memory_modification_parser.get_format_instructions()
            },
        )
        ...
        try:
            modification_plan = modification_chain.invoke(
                {"user_id": self.user_id, "conversation_turn": conversation_turn}
            )
            ...
            if vector_plan := modification_plan.get("vector_updates"):
                ...
                if docs_to_add_data := vector_plan.get("add_documents"):
                    ...
                    if docs_to_add:
                        ...
                        logger.info(
                            f"Added {len(docs_to_add)} new documents to vector store for user {self.user_id}."
                        )

        except Exception as e:
            logger.error(
                f"Failed to consolidate memories for user {self.user_id}: {e}",
                exc_info=True,
            )
    ...
    def _retrieve_short_term_context(self, user_input: str) -> str:
        ...
        try:
            docs = self.vector_store.similarity_search(
                user_input, k=3, filter={"user_id": self.user_id}
            )
            return (
            ...
            logger.warning(
                f"Could not retrieve from vector store for user {self.user_id}: {e}"
            )
    ...
    def _retrieve_long_term_context(self) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Could not retrieve from graph for user {self.user_id}: {e}"
            )
...
def generate_augmented_response(
    llm: BaseChatModel, short_term_context: str, long_term_context: str, user_input: str
) -> str:
